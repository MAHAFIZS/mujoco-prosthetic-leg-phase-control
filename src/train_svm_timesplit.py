...
def main():
    ...
    # Time-based split (windows are sequential)
    N = F.shape[0]
    ...
    print("\n=== TIME-SPLIT Classification report (0=swing, 1=stance) ===")
    print(classification_report(y_test, y_pred, digits=4))
    print("Confusion matrix:\n", confusion_matrix(y_test, y_pred))

    dump({"scaler": scaler, "clf": clf, "feat_names": feat_names}, outdir / f"svm_timesplit_{args.id}.joblib")
    ...
    (outdir / f"metrics_timesplit_{args.id}.json").write_text(json.dumps(metrics, indent=2))

    print(f"\nSaved model to: {outdir / f'svm_timesplit_{args.id}.joblib'}")
    print(f"Saved metrics to: {outdir / f'metrics_timesplit_{args.id}.json'}")
