...
from src.labels_proxy import label_contact_with_proxy
...
def main():
    ...
    lab = label_contact_with_proxy(d.t, d.imu, d.events, toggle_symbols=("/", "Q"))
    ...
    print("\n=== PROXY-LABEL TIME-SPLIT report (0=swing, 1=stance) ===")
    print(classification_report(y_test, y_pred, digits=4))
    print("Confusion matrix:\n", confusion_matrix(y_test, y_pred))

    dump({"scaler": scaler, "clf": clf, "feat_names": feat_names}, outdir / f"svm_proxy_{args.id}.joblib")
    ...
    (outdir / f"metrics_proxy_{args.id}.json").write_text(json.dumps(metrics, indent=2))
    print(f"\nSaved metrics to: {outdir / f'metrics_proxy_{args.id}.json'}")
